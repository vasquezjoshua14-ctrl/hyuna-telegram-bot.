    ...
    if (!ADMIN_ID) {
      console.warn("鈿狅笍 ADMIN_ID is not set. Admin commands will not work.");
    }
    ...
    const PRODUCTS = {
      gemini: {
        ...
        note: "鈿狅笍 No warranty after claim",
        deliveryType: "link",
      },
      ...
      chatgpt: {
        ...
        note: "馃洝 Full warranty 鈥� Manual account delivery up to 12 hours",
        deliveryType: "manual",
      },
      ...
      canva: {
        ...
        note: "馃摟 Send Gmail after payment 鈥� Manual delivery",
    ...
    function menuKeyboard() {
      return Markup.inlineKeyboard([
        [
          Markup.button.callback("馃泹 Products", "products"),
          Markup.button.callback("馃摝 My Orders", "my_orders"),
        ],
        [
          Markup.button.callback("馃挸 Payment Guide", "payment_guide"),
          Markup.button.url("馃摙 Channel", CHANNEL_URL),
        ],
        [Markup.button.url("馃摡 Contact Admin", `https://t.me/${ADMIN_USERNAME}`)],
      ]);
    }

    async function sendHome(ctx) {
      const caption =
        "馃尭 *Welcome to Hyuna Store!* 馃尭\n" +
        "Your cute & trusted digital shop 馃挆\n\n" +
    ...
    bot.action("products", async (ctx) => {
      ...
      let text =
        "馃尭鉁� *HYUNA STORE 鈥� AVAILABLE PRODUCTS* 鉁煂竆n" +
        "Choose your fave below 馃挆\n\n";

      for (const p of Object.values(PRODUCTS)) {
        text += `${p.emoji} *${p.name} 鈥� 鈧�${p.price}*\n`;
      ...
      text += "馃尭 _Please read the product details before ordering._";

      await ctx.reply(text, {
        parse_mode: "Markdown",
        ...Markup.inlineKeyboard([
          [Markup.button.callback("馃尫 Buy Gemini", "buy:gemini")],
          [Markup.button.callback("馃巰 Buy CapCut", "buy:capcut")],
          [Markup.button.callback("馃挄 Buy ChatGPT", "buy:chatgpt")],
          [Markup.button.callback("馃 Buy Canva", "buy:canva")],
          [Markup.button.callback("猬咃笍 Back to Menu", "home")],
        ]),
      });
    });
    ...
    bot.action('payment_guide', async (ctx) => {
      ...
      let caption =
        '馃挸 *PAYMENT GUIDE* 馃尭\n\n' +
        `馃摫 GCash Name: ${GCASH_NAME}\n` +
        `馃挵 GCash Number: ${GCASH_NUMBER}\n\n` +
        'Scan the QR code to pay.\n\n' +
        '馃挆 Keep your Order ID for reference.\n\n'
      ...
      if (order) {
        caption =
          ...
          `Price: *鈧�${order.pricePerItem} each*\n` +
          `Total: *鈧�${order.totalPrice}*\n\n` +
          caption
      }
    ...
    bot.action(/^send_receipt:(HYU-.+)$/, async (ctx) => {
      ...
      await ctx.reply(
        `馃摳 Please send your payment receipt screenshot here.\n\n` +
        `馃Ь Order ID: \`${id}\`\n\n` +
        `馃 Your receipt will be checked automatically.\n` +
    ...
    async function processReceiptMedia(ctx, media) {
      ...
      if (
        ...
        await ctx.reply(
          '鉂� Please send the receipt as a photo or image file.'
        )
        return true
      }
      ...
      if (!order) {
        ...
        await ctx.reply('鉂� Order not found.')
        return true
      }
      ...
      if (order.status !== 'waiting_payment') {
        ...
        await ctx.reply(
          '鉂� This order is no longer waiting for payment.'
        )
    ...
    bot.on('text', async (ctx, next) => {
      ...
      await ctx.reply(
        ...
        `馃捀 Price: *鈧�${product.price} each*\n` +

        `馃挵 Total: *鈧�${order.totalPrice}*\n\n` +

        `馃Ь Order ID: \`${order.id}\`\n\n` +

        `鈴� Waiting for Payment`,
    ...
    bot.hears(
      ...
      async (ctx) => {
        ...
        await ctx.reply(
          ...
          `馃尭 Preparing your Canva invite.`,
    ...
    bot.action(
      ...
      async (ctx) => {
        ...
        await ctx.reply(
          '馃尭 Thank you for confirming. Enjoy your order! 馃挄'
        )
    ...
    bot.command('admin', async (ctx) => {
      if (!isAdmin(ctx)) {
        return ctx.reply('鉂� Unauthorized')
      }
    ...
    bot.command('addaccount', async (ctx) => {
      ...
      `馃尭 HYUNA STORE UPDATE 馃尭\n\n` +
      `鉁� New stock available!\n\n` +
      `馃洅 ORDER NOW\n` +
      `https://t.me/AITOOLSHyuna_Bot?start=shop\n\n` +
      `馃挆 Thank you for supporting us!`
    );
        ...
        return ctx.reply(
          `鉁� Added ${links.length} Gemini links and notified users.`
        )
      }
    ...
    async function confirmPayment(order) {
      ...
      if (
        ...
        await bot.telegram.sendMessage(
          ...
          `馃挆 Payment confirmed!\n\n` +

          `Order: ${fresh.id}\n\n` +

          `馃尭 Your order is being prepared manually.`
